...
void world_types_to_zone_types(WORLD_DATA *world, ZONE_DATA *zone) {
    ...
    const char *key = NULL;
    ITERATE_HASH(key, type, type_i)zoneAddType(zone, key, type->read_func, type->store_func, type->delete_func,
                                               type->key_func);
...
void deleteWorld(WORLD_DATA *world) {
    ...
    ITERATE_HASH(key, type, type_i)deleteWorldTypeData(type);
    deleteHashIterator(type_i);
    deleteHashtable(world->type_table);

    // detach ourself from all of our zones
    ITERATE_HASH(key, zone, zone_i)zoneSetWorld(zone, NULL);
...
void worldPulse(WORLD_DATA *world) {
    ...
    ITERATE_HASH(key, zone, zone_i)zonePulse(zone);
    deleteHashIterator(zone_i);
}
...
void worldForceReset(WORLD_DATA *world) {
    ...
    ITERATE_HASH(key, zone, zone_i)zoneForceReset(zone);
    deleteHashIterator(zone_i);
}
...
LIST *worldGetZoneKeys(WORLD_DATA *world) {
    ...
    ITERATE_HASH(key, zone, zone_i)listQueue(keys, strdup(key));
    deleteHashIterator(zone_i);
    return keys;
}
...
void worldAddType(WORLD_DATA *world, const char *type, void *reader,
    ...
    if (!hashIn(world->type_table, type)) {
        ...
        const char *key = NULL;
        ZONE_DATA *zone = NULL;
        ITERATE_HASH(key, zone, zone_i)zoneAddType(zone, type, reader, storer, deleter, zonesetter);
        deleteHashIterator(zone_i);
    }
}
...
void worldAddForgetfulType(WORLD_DATA *world, const char *type, void *reader,
    ...
    if (!hashIn(world->type_table, type)) {
        ...
        const char *key = NULL;
        ZONE_DATA *zone = NULL;
        ITERATE_HASH(key, zone, zone_i)zoneAddForgetfulType(zone, type, reader, storer, deleter, zonesetter);
        deleteHashIterator(zone_i);
    }
}
