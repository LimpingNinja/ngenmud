...
void delete_storage_set(STORAGE_SET *set) {
    ...
    ITERATE_HASH(key, data, hash_i)delete_storage_data(data);
...
void write_storage_set(STORAGE_SET *set, FILEBUF *fb, int indent) {
    ...
    ITERATE_HASH(key, data, hash_i)listPut(elems, data);
    deleteHashIterator(hash_i);
...
void write_storage_list(STORAGE_SET_LIST *list, FILEBUF *fb, int indent) {
    ...
    ITERATE_LIST(set, list_i)write_storage_set(set, fb, indent);
    deleteListIterator(list_i);
}
...
STORAGE_SET_LIST *gen_store_list(LIST *list, void *storer) {
    ...
    ITERATE_LIST(elem, list_i)storage_list_put(set_list, store_func(elem));
    deleteListIterator(list_i);
    return set_list;
}
