...
void send_outdoors(const char *format, ...) {
    if (format && *format) {
        ...
        CHAR_DATA *ch = NULL;
        ITERATE_LIST(ch, list_i)if (charGetRoom(ch) != NULL &&
...
void send_to_list(LIST *list, const char *format, ...) {
    if (format && *format) {
        ...
        CHAR_DATA *ch = NULL;
        ITERATE_LIST(ch, list_i)text_to_char(ch, buf);
        deleteListIterator(list_i);
    }
};
...
//
// Send a message out
// ToDo: While some messages need $n, $N, $o, $O, $a, $A the gender specific
//       nessages can be them/their/they and a hook can be created for
//       parse_message so mudlib can handle genders without hard code.
//
