...
PyObject *mud_generic_find(PyObject *self, PyObject *args) {
    ...
    if (found_type == FOUND_CHAR) {
        ...
        if (IS_SET(type, FIND_TYPE_CHAR)) {
            ITERATE_LIST(one_found, found_i) PyList_Append(list, charGetPyFormBorrowed(one_found));
        } else if (IS_SET(type, FIND_TYPE_OBJ | FIND_TYPE_IN_OBJ)) {
            ITERATE_LIST(one_found, found_i) PyList_Append(list, objGetPyFormBorrowed(one_found));
        }
