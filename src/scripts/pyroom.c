...
int PyRoom_init(PyRoom *self, PyObject *args, PyObject *kwds) {
    ...
    if (PyInt_Check(who)) {
        ...
        ROOM_DATA *room = NULL;
        if ((room = worldGetRoom(gameworld, get_fullkey_relative(PyString_AsString(who), get_script_locale()))) !=
            NULL) {
...
PyObject *PyRoom_getchars(PyRoom *self, PyObject *args) {
    ...
    if (room == NULL)
        ...
        ITERATE_LIST(ch, char_i)PyList_Append(list, charGetPyFormBorrowed(ch));
...
PyObject *PyRoom_getobjs(PyRoom *self, PyObject *args) {
    ...
    if (room == NULL)
        ...
        ITERATE_LIST(obj, obj_i)PyList_Append(list, objGetPyFormBorrowed(obj));
...
init_PyRoom(void) {
    ...
    deleteListWith(pyroom_getsetters, free);
    pyroom_getsetters = NULL;
    deleteListWith(pyroom_methods, free);
    pyroom_methods = NULL;
