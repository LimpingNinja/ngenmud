...
void deleteRoom(ROOM_DATA *room) {
    ...
    ITERATE_LIST(content, cont_i)extract_obj(content);
    deleteListIterator(cont_i);

    cont_i = newListIterator(room->characters);
    ITERATE_LIST(content, cont_i)extract_mobile(content);
    deleteListIterator(cont_i);
    ...
    const char *dir = NULL;
    EXIT_DATA *ex = NULL;
    ITERATE_HASH(dir, ex, ex_i)deleteExit(ex);
    ...
    if (room->cmd_table != NULL) {
        ...
        const char *key = NULL;
        CMD_DATA *cmd = NULL;
        ITERATE_NEARMAP(key, cmd, cmd_i)deleteCmd(cmd);
