...
bool text_to_socket(SOCKET_DATA *dsock, const char *txt) {
    ...
    if (dsock && dsock->out_compress) {
        ...
        while (dsock->out_compress->avail_in) {
            dsock->out_compress->avail_out =
                    COMPRESS_BUF_SIZE - (dsock->out_compress->next_out - dsock->out_compress_buf);
...
void reconnect_copyover_sockets() {
    ...
    ITERATE_LIST(sock, sock_i)FD_SET(sock->control, &fSet);
    deleteListIterator(sock_i);
}
...
void do_copyover(void) {
    ...
    // TODO: Evaluate, this seems awkwardly hard coded
    execl(EXE_FILE, "ngenmud", "-copyover", control_buf, port_buf, NULL);
}
