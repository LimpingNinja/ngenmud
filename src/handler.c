...
void obj_exist(OBJ_DATA *obj) {
    ...
    if (listSize(objGetContents(obj)) > 0) {
        ...
        OBJ_DATA *cont = NULL;
        ITERATE_LIST(cont, cont_i)obj_exist(cont);
...
void obj_unexist(OBJ_DATA *obj) {
    ...
    if (listSize(objGetContents(obj)) > 0) {
        LIST_ITERATOR *cont_i = newListIterator(objGetContents(obj));
        OBJ_DATA *cont = NULL;
        ITERATE_LIST(cont, cont_i)obj_unexist(cont);
        deleteListIterator(cont_i);
    }
...
void obj_to_game(OBJ_DATA *obj) {
    ...
    if (listSize(objGetContents(obj)) > 0) {
        LIST_ITERATOR *cont_i = newListIterator(objGetContents(obj));
        OBJ_DATA *cont = NULL;
        ITERATE_LIST(cont, cont_i)obj_to_game(cont);
        deleteListIterator(cont_i);
    }
}
...
void room_exist(ROOM_DATA *room) {
    ...
    if (listSize(roomGetCharacters(room)) > 0) {
        ...
        CHAR_DATA *ch = NULL;
        ITERATE_LIST(ch, ch_i)char_exist(ch);
...
void room_unexist(ROOM_DATA *room) {
    ...
    if (listSize(roomGetCharacters(room)) > 0) {
        LIST_ITERATOR *ch_i = newListIterator(roomGetCharacters(room));
        CHAR_DATA *ch = NULL;
        ITERATE_LIST(ch, ch_i)char_unexist(ch);
        deleteListIterator(ch_i);
    }
...
void room_to_game(ROOM_DATA *room) {
    ...
    if (listSize(roomGetCharacters(room)) > 0) {
        LIST_ITERATOR *ch_i = newListIterator(roomGetCharacters(room));
        CHAR_DATA *ch = NULL;
        ITERATE_LIST(ch, ch_i)char_to_game(ch);
        deleteListIterator(ch_i);
    }
...
void char_exist(CHAR_DATA *ch) {
    ...
    if (listSize(charGetInventory(ch)) > 0) {
        ...
        OBJ_DATA *obj = NULL;
        ITERATE_LIST(obj, inv_i)obj_exist(obj);
    ...
    if (listSize(eq) > 0) {
        ...
        OBJ_DATA *obj = NULL;
        ITERATE_LIST(obj, eq_i)obj_exist(obj);
...
void char_unexist(CHAR_DATA *ch) {
    ...
    if (listSize(charGetInventory(ch)) > 0) {
        LIST_ITERATOR *inv_i = newListIterator(charGetInventory(ch));
        OBJ_DATA *obj = NULL;
        ITERATE_LIST(obj, inv_i)obj_unexist(obj);
        deleteListIterator(inv_i);
    }
    ...
    if (listSize(eq) > 0) {
        LIST_ITERATOR *eq_i = newListIterator(eq);
        OBJ_DATA *obj = NULL;
        ITERATE_LIST(obj, eq_i)obj_unexist(obj);
        deleteListIterator(eq_i);
    }
    deleteList(eq);
...
void char_to_game(CHAR_DATA *ch) {
    ...
    if (listSize(charGetInventory(ch)) > 0) {
        LIST_ITERATOR *inv_i = newListIterator(charGetInventory(ch));
        OBJ_DATA *obj = NULL;
        ITERATE_LIST(obj, inv_i)obj_to_game(obj);
        deleteListIterator(inv_i);
    }
    ...
    if (listSize(eq) > 0) {
        LIST_ITERATOR *eq_i = newListIterator(eq);
        OBJ_DATA *obj = NULL;
        ITERATE_LIST(obj, eq_i)obj_to_game(obj);
        deleteListIterator(eq_i);
    }
    deleteList(eq);
}
...
void obj_from_game(OBJ_DATA *obj) {
    ...
    if (listSize(objGetContents(obj)) > 0) {
        LIST_ITERATOR *cont_i = newListIterator(objGetContents(obj));
        OBJ_DATA *cont = NULL;
        ITERATE_LIST(cont, cont_i)obj_from_game(cont);
        deleteListIterator(cont_i);
    }
...
void room_from_game(ROOM_DATA *room) {
    ...
    if (listSize(roomGetCharacters(room)) > 0) {
        LIST_ITERATOR *ch_i = newListIterator(roomGetCharacters(room));
        CHAR_DATA *ch = NULL;
        ITERATE_LIST(ch, ch_i)char_from_game(ch);
        deleteListIterator(ch_i);
    }
    ...
    LIST_ITERATOR *ex_i = newListIterator(ex_list);
    char *dir = NULL;
    ITERATE_LIST(dir, ex_i)exit_from_game(roomGetExit(room, dir));
    deleteListIterator(ex_i);
    deleteListWith(ex_list, free);
...
void char_from_game(CHAR_DATA *ch) {
    ...
    if (listSize(charGetInventory(ch)) > 0) {
        LIST_ITERATOR *inv_i = newListIterator(charGetInventory(ch));
        OBJ_DATA *obj = NULL;
        ITERATE_LIST(obj, inv_i)obj_from_game(obj);
        deleteListIterator(inv_i);
    }
    ...
    if (listSize(eq) > 0) {
        LIST_ITERATOR *eq_i = newListIterator(eq);
        OBJ_DATA *obj = NULL;
        ITERATE_LIST(obj, eq_i)obj_from_game(obj);
        deleteListIterator(eq_i);
    }
    deleteList(eq);
